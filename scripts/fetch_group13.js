...
const TABLE_PATH = path.join(__dirname, '../tables/table13.json');
...
// -----------------------------------------------------------------------------
(async () => {
  console.log('🚀 Démarrage du script Group 13 Handball...');
  ...
  try {
    ...
    console.error('❌ Erreur lecture table13.json :', err.message);
    process.exit(2);
  }
...
console.log(`   ✅ ${matchesTomorrow.length} match(s) Top3 à domicile à venir.`);
  ...
  await browser.close();
  console.log('\n🏁 Fin du script Group 13 HandBall.');
})();
