// scripts/fetch_group12.js
// Analyse Group 12 hockey : Top 3 à domicile + dernier match perdu → envoi au VPS
// Usage : node scripts/fetch_group12.js
...
const TABLE_PATH = path.join(__dirname, '../tables/table12.json');
...
// -----------------------------------------------------------------------------
(async () => {
  console.log('🚀 Démarrage du script Group 12 Hockey...');
  ...
  try {
    ...
    console.error('❌ Erreur lecture table12.json :', err.message);
    process.exit(2);
  }
...
console.log(`   ✅ ${matchesTomorrow.length} match(s) Top3 à domicile à venir.`);
      ...
      for (const m of matchesTomorrow) {
        ...
        if (result === 'loss') {
          console.log('       ✅ Équipe a perdu son dernier match → ajoutée à la liste.');
          matchesToSend.push({
            country,
            league: leagueName,
            leagueId,
